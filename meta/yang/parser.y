...
description : kywd_description token_string {
        yylval.stack.Peek().(meta.Describable).SetDescription(tokenString($2))
    }
...
choice_def :
    kywd_choice token_ident {
        yylval.stack.Push(&meta.Choice{Ident:$2})
    };
...
case_def :
    kywd_case token_ident {
        yylval.stack.Push(&meta.ChoiceCase{Ident:$2})
    };
...
typedef_def :
    kywd_typedef token_ident {
        yylval.stack.Push(&meta.Typedef{Ident:$2})
    };
...
type_stmt_def : kywd_type token_ident {
            y := yylval.stack.Peek().(meta.HasDataType)
            yylval.dataType = meta.NewDataType(y, $2)
        };
...
container_def :
    kywd_container token_ident {
        yylval.stack.Push(&meta.Container{Ident:$2})
    };
...
uses_stmt :
     kywd_uses token_ident token_semi {
         yylval.stack.Push(&meta.Uses{Ident:$2})
         if HasError(yylex, popAndAddMeta(&yylval)) {
           goto ret1
         }
     }
...
rpc_def :
    kywd_rpc token_ident {
        yylval.stack.Push(&meta.Rpc{Ident:$2})
    };
...
grouping_def :
    kywd_grouping token_ident {
        yylval.stack.Push(&meta.Grouping{Ident:$2})
    };
...
list_def :
    kywd_list token_ident {
        yylval.stack.Push(&meta.List{Ident:$2})
    };
...
anyxml_def :
    kywd_anyxml token_ident {
        yylval.stack.Push(meta.NewAny($2))
    }
...
leaf_def :
    kywd_leaf token_ident {
        yylval.stack.Push(&meta.Leaf{Ident:$2})
    };
...
leaf_list_def :
    kywd_leaf_list token_ident {
        yylval.stack.Push(&meta.LeafList{Ident:$2})
    };
