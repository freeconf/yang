...
func ExampleBasic_onField() {
	...
	sel.UpsertFrom(nodes.ReadJSON(`{"foo":"WRITE"}`))
	// Output:
	// {"foo":"READ"}
	// WRITE
}
...
func ExampleBasic_onChild() {
	...
	sel.InsertFrom(nodes.ReadJSON(`{"foo":{"bar":"y"}}`))
	examplePrint(sel)
...
func ExampleBasic_onNext() {
	...
	sel := exampleSelection(model, &nodes.Basic{
		OnChild: func(r node.ChildRequest) (node.Node, error) {
			return dataList(), nil
		},
	})
	...
	fmt.Println("Creating")
	sel.UpsertFrom(nodes.ReadJSON(`{"foo":[{"bar":"b"}]}`))
	examplePrint(sel)
...
func ExampleBasic_onAction() {
	...
	result := sel.Action(nodes.ReadJSON(`{"a":10,"b":32}`))
	examplePrint(result)
