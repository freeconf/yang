...
func (self ConfigProxy) Node(a node.Node, b node.Node) node.Node {
	var edit node.Node
	return &Basic{
...
func (self ConfigProxy) createEdit(r node.NodeRequest, a node.Node, b node.Node) (node.Node, error) {
	params := "depth=1&content=config&with-defaults=trim"
	data := make(map[string]interface{})
	edit := ReflectChild(data)
