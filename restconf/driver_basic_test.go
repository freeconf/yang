...
type testDriverSupport struct {
	...
	_subs       map[string]*driverSub
	ws          bytes.Buffer
	subPayloads bytes.Buffer
}

func (self *testDriverSupport) reset() *driver {
	self._log = ""
	self._subs = make(map[string]*driverSub)
...
func (self *testDriverSupport) driverDo(method string, params string, p *node.Path, payload io.Reader) (node.Node, error) {
...
func (self *testDriverSupport) driverSubs() map[string]*driverSub {
	return self._subs
}

func (self *testDriverSupport) driverWebsocket() (io.Writer, error) {
	return &self.ws, nil
}
